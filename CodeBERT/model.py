...
class CodeBERTModel(nn.Module):  # Model类继承自nn.Module，是一个用于代码漏洞检测的模型。它封装了一个编码器，用于分析代码并预测其是否包含漏洞
    ...
    def forward(self, input_ids=None, labels=None, inputs_embeds=None, attention_mask=None):  # 模型的核心，定义了前向传播过程
        ...
        if inputs_embeds is not None:
            ...
            outputs = self.encoder(input_ids, attention_mask=attention_mask)[0]  # 调用self.encoder处理输入，获取编码后的表示
        """
        ...
        if labels is not None:  # 如果提供了标签（训练或评估模式）
            ...
            loss = self.loss_func(logits, labels)  # 直接使用 logits 和原始 labels
            """
